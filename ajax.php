<?php
// Токен телеграм бота
$tg_bot_token = "6818232146:AAEC-fv17g-zKVSgymaZsf_dYjGExnD2h7Y";
// ID Чата
$chat_id = "-4023648979";

$responseBody = [];
$responseBody['success'] = false;
$responseBody['message'] = '';

if ($_POST['user_name'] && $_POST['user_email']) {
    $text = 'Поступила новая заявка с сайта' . "\n";

    foreach ($_POST as $key => $val) {
        $text .= $key . ": " . $val . "\n";
    }

    $text .= "\n" . "IP-адрес клиента: " . $_SERVER['REMOTE_ADDR'];
    $text .= "\n" . "Дата отправки заявки: " . date('d.m.y H:i:s');
    $param = [
        "chat_id" => $chat_id,
        "text" => $text
    ];
    $url = "https://api.telegram.org/bot" . $tg_bot_token . "/sendMessage?" . http_build_query($param);

    file_get_contents($url);

    foreach ($_FILES as $file) {

        $url = "https://api.telegram.org/bot" . $tg_bot_token . "/sendDocument";

        move_uploaded_file($file['tmp_name'], $file['name']);

        $document = new \CURLFile($file['name']);

        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, ["chat_id" => $chat_id, "document" => $document]);
        curl_setopt($ch, CURLOPT_HTTPHEADER, ["Content-Type:multipart/form-data"]);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);

        $out = curl_exec($ch);

        curl_close($ch);

        unlink($file['name']);
    }
    $responseBody['success'] = true;
    $responseBody['message'] = 'Form submitted successfully.';
} else {
    $responseBody['message'] = 'Error: Name and Email are required fields.';
}

header('Content-Type: application/json');
header('HTTP/1.1 200 OK');
echo json_encode($responseBody);
exit();
